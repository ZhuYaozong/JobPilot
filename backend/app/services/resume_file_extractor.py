...
Bridges the gap between "user has a PDF/DOCX/MD/TXT on disk" and the rest of
JobPilot's pipeline which expects a single ``raw_text`` field. After this
runs, the extracted text is fed into the existing
``resume_parsing_service.parse_resume`` flow exactly like a manually-pasted
resume would be.
...
files surface a 400 to the user with a friendly message.
"""
...
# 5 MB cap — comfortably above a typical text-heavy resume (≈100 KB) but
# small enough that we never have to chunk uploads or worry about request
# memory. Bumped per-deployment via env later if real users hit the wall.
...
# to LLM parsing with garbage input.
...
def extract_text_from_upload(
    ...
    if len(payload) > MAX_RESUME_BYTES:
        size_mb = MAX_RESUME_BYTES // (1024 * 1024)
        raise ResumeExtractionError(
            f"文件超过 {size_mb} MB 上限，请压缩或截取后再上传。",
        )

    if not payload:
        raise ResumeExtractionError("上传的文件是空的。")
    ...
    return ExtractedResume(text=normalised, source_type=source)
...
def _extract_docx(payload: bytes) -> str:
    ...
    newlines so the LLM parsing prompt sees structure roughly equivalent to
    "Resume text: <body>".
    """
...
def _decode_text(payload: bytes) -> str:
    """Decode UTF-8 with a graceful fallback to GBK.

    Most resumes posted online are UTF-8 these days, but legacy Windows
    exports from Chinese-locale machines can still be GBK; trying both keeps
    those users from hitting a wall.
    """
    for encoding in ("utf-8", "gbk", "utf-16"):
        try:
            ...
            continue
    raise ResumeExtractionError("文件编码不支持，请保存为 UTF-8 后重试。")


def _normalise_whitespace(text: str) -> str:
    """Trim each line + collapse runs of blank lines.

    LLM token budget is precious; PDFs especially tend to insert empty lines
    between every paragraph. Collapsing them costs nothing and shortens the
    parsing prompt noticeably.
    """
