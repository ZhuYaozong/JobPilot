...
测试会连接当前后端配置中的 PostgreSQL 数据库。由于当前项目还没有独立测试数据库配置，测试通过 API 创建带 `pytest-jobpilot-*` 唯一标记的前置数据，不依赖已有人工数据或固定 id。当前测试不会覆盖完整系统，只覆盖关键 workflow、AI 产物承载闭环和 JD 解析闭环。JD 解析测试使用 monkeypatch 替代真实 LLM 调用。

## AI 配置

JD 结构化解析需要配置一个 OpenAI-compatible chat completions 接口：
...
`LLM_BASE_URL` 会拼接 `/chat/completions` 发起请求。缺少配置时，parse 接口会返回清晰错误，不会伪造成功结果。
...
- 没有做 Resume 解析、MatchResult 自动生成、Cover Letter 自动生成或面试题生成。
