...
JobPilot 后端是面向求职场景的 Agentic AI Workflow 系统的最小承载层。当前已经完成应用骨架、配置管理、PostgreSQL 异步访问、SQLAlchemy、Alembic、健康检查、Resume / JobPosting / MatchResult / ApplicationRecord 四个核心模块的最小 API 闭环、workflow 状态层、AI 产物承载层，以及 JD 结构化解析、Resume 结构化解析、MatchResult 自动生成和 Cover Letter 草稿生成能力。
...
当前仍不包含 RAG、LangChain、LangGraph、认证登录、前端、文件上传、Redis 业务逻辑、自动提醒或复杂状态机。
...
测试会连接当前后端配置中的 PostgreSQL 数据库。由于当前项目还没有独立测试数据库配置，测试通过 API 创建带 `pytest-jobpilot-*` 唯一标记的前置数据，不依赖已有人工数据或固定 id。当前测试不会覆盖完整系统，只覆盖关键 workflow、AI 产物承载闭环、JD 解析闭环、Resume 解析闭环、MatchResult 自动生成闭环和 Cover Letter 草稿生成闭环。JD / Resume 解析、MatchResult 自动生成和 Cover Letter 草稿生成测试使用 monkeypatch 替代真实 LLM 调用。
...
`LLM_BASE_URL` 会拼接 `/chat/completions` 发起请求。缺少配置时，parse / analyze / generate 接口会返回清晰错误，不会伪造成功结果。没有真实 API Key 时，仍可运行自动化测试，因为测试会 mock LLM 调用。
...
- `generated_artifacts`：保存求职信、简历摘要等 AI 产物承载记录，目前支持手工写入、更新，以及 Cover Letter 草稿生成写入。
...
- 没有接入除 Cover Letter 草稿外的其他真实 AI 生成逻辑。
- 没有做模板渲染、PDF/DOCX 导出或产物删除接口。
- 没有引入 LangChain、LangGraph 或 RAG。
- 没有做简历改写/定制生成、面试题生成或多种 artifact 一起生成。
