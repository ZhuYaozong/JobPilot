...
JobPilot 后端是面向求职场景的 Agentic AI Workflow 系统的最小承载层。当前已经完成应用骨架、配置管理、PostgreSQL 异步访问、SQLAlchemy、Alembic、健康检查、Resume / JobPosting / MatchResult / ApplicationRecord 四个核心模块的最小 API 闭环、workflow 状态层、AI 产物承载层，以及 JD 结构化解析、Resume 结构化解析、MatchResult 自动生成、Cover Letter 草稿生成、Interview Prep 生成和 AI 产物反馈记录能力。
