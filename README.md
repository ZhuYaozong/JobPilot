...
JobPilot 是一个面向求职场景的 Agentic AI Workflow 系统。当前已经完成本地基础设施、后端 FastAPI 骨架、SQLAlchemy async、Alembic、Resume / JobPosting / MatchResult / ApplicationRecord 四个核心模块的最小 API 闭环、workflow 状态层、AI 产物承载层，以及 JD 结构化解析、Resume 结构化解析、MatchResult 自动生成、Cover Letter 草稿生成和 Interview Prep 生成能力。
...
当前测试会连接后端配置中的 PostgreSQL 数据库，通过 API 创建带 `pytest-jobpilot-*` 标记的前置数据，不依赖数据库里已有的固定记录。JD / Resume 解析、MatchResult 自动生成、Cover Letter 草稿生成和 Interview Prep 生成测试使用 monkeypatch 替代真实 LLM 调用。
