...
第 26 步继续做纯前端产品化整改：首页已重构为任务导向工作台，导航表达也更贴近用户任务入口，而不是把系统分层术语放在第一视野。
...
- 首页从“系统说明台”调整为“任务导向工作台”，优先展示主任务入口、最近处理、待推进事项和 AI 快捷入口
- 导航分组弱化系统分层术语，改为更贴用户任务的软件导航表达
- 这一步不改后端、不改 API 主干、不改页面路由主干，也不接真实 Agent / RAG
...
- `/`：Dashboard 任务导向首页，展示主任务入口、最近处理、待推进事项、AI 快捷入口与轻量说明
...
- Dashboard 首页：真实加载最近 JobPosting / Resume / MatchResult / GeneratedArtifact / ApplicationRecord，并以任务导向方式组织主入口、最近处理与 AI 快捷入口
...
第 24 步后，这些页面共享同一个最小用户 header 机制；切换 `demo / sandbox` 后，页面会重新加载当前用户作用域下的数据。第 25 步进一步把主要页面文案收口为中文优先，并把五个工作页的左侧列表收成统一的受控高度和内部滚动区域。第 26 步再把首页和导航重构成更像中文求职软件的任务入口。
