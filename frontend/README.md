...
- `/assistant`：AI Copilot Layer 高保真产品壳，已升级为三栏 Copilot 工作区，展示推荐场景、会话预览、对话壳、上下文插槽、工具预览与 Knowledge 入口，但尚未接真实 Agent Runtime
- `/knowledge`：高保真 Knowledge / RAG 入口壳，已升级为多区知识工作台，展示 Private Knowledge、General Knowledge、Retrieval Flow、Context Injection、Citations / Trace 意识与 Assistant 协同关系，但尚未接真实检索运行时
...
第 24 步后，这些页面共享同一个最小用户 header 机制；切换 `demo / sandbox` 后，页面会重新加载当前用户作用域下的数据。
